...
fn setup_test_env() -> (TempDir, String, String) {
    ...
    db::apply_all_migrations(&mut conn).unwrap();
...
#[test]
fn crash_point_7_multipart_interrupted_part_tmp() {
    let (_dir, db_path, spool_dir) = setup_test_env();
    let conn = db::open(&db_path).unwrap();
...
#[test]
fn crash_point_8_multipart_part_renamed_before_db_commit() {
    let (_dir, db_path, spool_dir) = setup_test_env();
    let conn = db::open(&db_path).unwrap();
...
#[test]
fn crash_point_9_abort_multipart_cleans_spool() {
    let (_dir, db_path, spool_dir) = setup_test_env();
    let mut conn = db::open(&db_path).unwrap();
    ...
    let spool_paths = db::abort_multipart_upload(&mut conn, upload_id).unwrap();
...
#[test]
fn crash_point_10_complete_multipart_crash_recovery() {
    let (_dir, db_path, spool_dir) = setup_test_env();
    let mut conn = db::open(&db_path).unwrap();
    ...
    let (_old_spools, version) =
        db::complete_multipart_upload_txn(&mut conn, upload_id, version_id, &req_parts, job_id)
            .unwrap();
    ...
    let chunks = db::chunks_of(&conn, "v-mp-10").unwrap();
    ...
    let chunks_after = db::chunks_of(&conn, "v-mp-10").unwrap();
    assert_eq!(chunks_after[0].state, "remote");
