...
#[test]
fn test_integration_doctor_recovery_bundle_and_scrub() {
    ...
    create_bucket(&conn, "doc-bucket", "telecrate-1").unwrap();
    ...
    export_recovery_bundle_file(&conn, bundle_file.to_str().unwrap(), None).unwrap();
