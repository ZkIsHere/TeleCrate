...
fn test_high_density_scale_simulation_25k_objects() {
    ...
    let rows = telecrate::db::list_keys(&conn, bucket, "folder_1/", "", 1000).unwrap();
    let list_dur = start_list.elapsed();
    println!("--> Benchmark ListObjectsV2 (1000 items): {:.2?}", list_dur);
    assert_eq!(rows.len(), 1000);
    assert!(
        list_dur.as_millis() < 100,
        "ListObjectsV2 latency exceeds 100ms"
    );

    // Benchmark 2: ListObjectVersions
    let start_versions = Instant::now();
    let versions =
        telecrate::db::list_object_versions(&conn, bucket, "folder_1/", "", "", 500).unwrap();
    ...
    assert!(
        versions_dur.as_millis() < 100,
        "ListObjectVersions latency exceeds 100ms"
    );
    ...
    for k in &to_delete {
        let _ = telecrate::db::delete_object(conn_mut, bucket, k);
    }
    let del_dur = start_del.elapsed();
    println!("--> Benchmark Batch Deletion 100 items: {:.2?}", del_dur);
    assert!(
        del_dur.as_millis() < 200,
        "DeleteObjects batch latency exceeds 200ms"
    );

    // Benchmark 4: active_spool_paths on 25k DB
    let start_spool = Instant::now();
    let active_spools = telecrate::db::active_spool_paths(conn_mut).unwrap();
    ...
    assert!(
        spool_dur.as_millis() < 100,
        "active_spool_paths latency exceeds 100ms"
    );
